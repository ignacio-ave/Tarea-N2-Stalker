...
void agregarPuntos(HashMap *jugadores) {
  ...
  scanf("%ld" , &puntos) ; 
  if(puntos<=0){
    printf("Puntos a asignar invalidos\n");
    return;
  }
...
void mostrarItemJugadores(HashMap *jugadores, HashMap *itemsMap){
  ...
  //uso de funciones en printformato debido a posible acento en los items
  int espaciositemmedio=(58-printInventario(item))/2;

  //se ve si el numero de letras es impar o par para ver como encaja el formato del print
...
void deshacerUltimaAccion(HashMap *jugadores, HashMap *itemsMap) {
  ...
  switch (ultima->ultimaAcc) {
    ...
    case 5 :
      ...
      printf("Se le han restaron %lu puntos de habilidad a el jugador %s\n",ultima->ptosH,jugador->key);
...
void exportarDatos(HashMap *jugadores){
  ...
  //Se crea el archivo con el nombre designado
  ...
  //Se imprime el formato que tendra el archivo para guirse
  ...
  //Se empiezan a recorrer los jugadores dentro del mapa y a imprimir sus valores
  ...
  while(local!=NULL && local->key!=NULL){
    ...
    //Abrimos el mapa de items de cada jugador si es que no esta vacio lo recorremos e imprimimos los items
...
//Esta funcion fue un poco de experimentacion ya que pudimos haberlo hecho de una manera mas facil pero como no vimos muy en profundidad archivos queriamos probar por que seria distinto hacerlo con un fscanf envez de un fgetline, tuvimos bastantes errores por los caracters invisibles q podrian estar dentro del archivo pero creo q se manejaron bien almenos los que venia el archivo players10000.csv (como el retorno de carro)
void importacion(FILE *archivo, HashMap *jugadores, HashMap *itemsMap){
...
//Se busca abre el archivo segun la ruta designada y se llama la funcion importacion
void importarJugadores(HashMap *jugadores, HashMap *itemsMap){
  ...
  importacion(archivo,jugadores, itemsMap);
